...
int System::addParticle(Particle* particle) {
	//add the element
...
int System::addElement(Element* element) {
	//add the element
...
int System::addForce(Element* element, double xi, float3 force) {
...
int System::updatePhiq() // used in Newton iteration, nice to keep it separate (but not memory efficient) - only needs to be done once (linear constraints)
{
...
int System::updateParticleDynamics() {
...
int System::calculateInitialPhi() {
	for (int i = 0; i < constraints.size(); i++)
		phi0_h.push_back(0);
	for (int i = 0; i < constraints.size(); i++) {
		Constraint constraint = constraints[i];
...
int System::initializeSystem() {
	...
	initializeDevice();
	//System::initializeBoundingBoxes_CPU();
...
float3 System::getXYZPositionParticle(int index) {
	return make_float3(pParticle_h[3 * index], pParticle_h[3 * index + 1],
			pParticle_h[3 * index + 2]);
}

float3 System::getXYZVelocityParticle(int index) {
