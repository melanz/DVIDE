...
int Beam::addBeam(int j)
{
  ...
  sys->f_h.push_back(rho * A * sys->gravity.x / 0.2e1);
  sys->f_h.push_back(rho * A * sys->gravity.y / 0.2e1);
  sys->f_h.push_back(rho * A * sys->gravity.z / 0.2e1);
  sys->f_h.push_back(rho * A * l * sys->gravity.x / 0.12e2);
  sys->f_h.push_back(rho * A * l * sys->gravity.y / 0.12e2);
  sys->f_h.push_back(rho * A * l * sys->gravity.z / 0.12e2);
  sys->f_h.push_back(rho * A * sys->gravity.x / 0.2e1);
  sys->f_h.push_back(rho * A * sys->gravity.y / 0.2e1);
  sys->f_h.push_back(rho * A * sys->gravity.z / 0.2e1);
  sys->f_h.push_back(-rho * A * l * sys->gravity.x / 0.12e2);
  ...
  for(int i=0;i<contactGeometry.z;i++) {
    ...
    sys->collisionMap_h.push_back(make_int4(identifier,i,0,collisionFamily));
  }
