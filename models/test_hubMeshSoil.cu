...
System* sys;
std::string outDir = "../TEST_HUBMESHSOIL/";
...
int main(int argc, char** argv)
{
  ...
  if(argc > 1) {
    ...
    numDivW = atoi(argv[3]);
    slip = atof(argv[4]);
    frictionCoefficient = atof(argv[5]);
  ...
  double ditchWidth = 6.0*beltWidth;
  ...
  if(mode==0) {
    ...
    sys->importSystem("../hubMeshSoil.dat");
