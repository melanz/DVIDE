...
System* sys;
std::string outDir = "../TEST_COMPACTION/";
...
int main(int argc, char** argv)
{
  ...
  outDirStream << "../TEST_COMPACTION_n" << numElementsPerSide << "_h" << hh << "_tol" << tolerance << "_sol" << solverTypeQOCC << "/";
  ...
  topPtr->setBodyFixed(false);
  topPtr->setGeometry(make_double3(10*0.5*numElementsPerSide+radius,radius,10*0.5*numElementsPerSide+radius));
  topPtr->setMass(numElementsPerSideY*numElementsPerSide*numElementsPerSide);
  sys->add(topPtr);
...
//  sys->add(bodyPtr);
  ...
  statsFileStream << outDir << "statsCompaction_n" << numElementsPerSide << "_h" << hh << "_tol" << tolerance << "_sol" << solverTypeQOCC << ".dat";
